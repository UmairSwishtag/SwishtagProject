...
export function SyncManagementPanel() {
  ...
  const handleSync = () => {
    ...
    // Every 4th sync fails for demo purposes
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-gradient-to-r from-slate-50 to-white">
        ...
        {/* Status Pill */}
        <span
          className={`flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full border ring-1 ${cfg.bgColor} ${cfg.textColor} ${cfg.ringColor}`}
        >
          <StatusIcon
            size={11}
            className={`${cfg.iconColor} ${status === 'syncing' ? 'animate-spin' : ''}`}
          />
          {cfg.label}
        </span>
      </div>
      ...
      {/* Progress Bar (visible during syncing) */}
      {status === 'syncing' && (
        <div className="px-4 pt-3">
          ...
          <p className="text-xs text-gray-400 mt-1.5">
            Fetching from Shopify GraphQL API…
          </p>
        </div>
      )}
      ...
      <div className="border-t border-gray-100">
        ...
        >
          ...
          <ChevronDown
            size={13}
            className={`text-gray-400 transition-transform duration-200 ${showLogs ? 'rotate-180' : ''}`}
          />
        </button>
