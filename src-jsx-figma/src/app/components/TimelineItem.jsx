            ...
            <div className="relative flex-shrink-0">
              <div className="w-11 h-11 rounded-lg overflow-hidden bg-gray-100 border border-gray-200">
                <img
                  ...
                  onError={(e) => {
                    e.target.src = 'https://via.placeholder.com/44x44?text=?';
                  }}
                />
              </div>
              <div
                className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border border-white flex items-center justify-center ${
                  item.changeType === 'price'
                    ? 'bg-emerald-500'
                    : item.changeType === 'inventory'
                    ? 'bg-blue-500'
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <p className="text-sm text-gray-900 truncate leading-tight">
                    {item.productName}
                  </p>
                  {item.sku && (
              ...
              <div className="flex items-center gap-1.5 mt-2">
                <span className="text-xs text-gray-400 line-through px-2 py-0.5 bg-gray-50 rounded border border-gray-100">
                  {item.oldValue}
                </span>
