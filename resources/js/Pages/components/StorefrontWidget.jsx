            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  {notif.delta && (
                    <span className={`flex items-center gap-0.5 text-xs ${notif.delta.direction === 'down' ? 'text-red-500' : 'text-emerald-600'}`}>
...
export function StorefrontInlineWidget({ message, timestamp, type = 'price' }) {
  ...
  return (
    <div className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg border ${type === 'price' ? 'bg-emerald-50 border-emerald-200' : type === 'inventory' ? 'bg-blue-50 border-blue-200' : 'bg-purple-50 border-purple-200'}`}>
      <Icon size={13} className={type === 'price' ? 'text-emerald-600' : type === 'inventory' ? 'text-blue-600' : 'text-purple-600'} />
      <span className={`text-xs ${type === 'price' ? 'text-emerald-800' : type === 'inventory' ? 'text-blue-800' : 'text-purple-800'}`}>
